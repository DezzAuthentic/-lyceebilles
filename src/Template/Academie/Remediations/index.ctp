<?php
    ...
    foreach($remediations as $remediation){
        $eleves_i[$remediation->inscription->eleve_id]['id'] = $remediation->inscription->id;
        $eleves_i[$remediation->inscription->eleve_id]['nom'] = $remediation->inscription->elef->nom;
        $eleves_i[$remediation->inscription->eleve_id]['prenom'] = $remediation->inscription->elef->prenom;
        $eleves_i[$remediation->inscription->eleve_id]['matricule'] = $remediation->inscription->elef->matricule;
        $eleves_i[$remediation->inscription->eleve_id]['promotion'] = $remediation->inscription->promotion->nom;
        $eleves_i[$remediation->inscription->eleve_id]['matieres'][] = $remediation->matiere->nom." ("."Pr. ".$remediation->professeur->prenom." ".$remediation->professeur->nom.")";
    }
    //dd($eleves_i);
?>
<div class="titre">
    <span>Gestion des remédiations</span>
...
<div class="row">
    ...
    <div class="col-xs-12">
        <table id="remediations_table" class="table datatable hover compact" >
            <thead>
                <tr>
                    <th>#</th>
                    <th title="Nom">Nom</th>
                    ...
                    <th title="Matières testées">Matières en remédiations</th>
                    <th class="actions" style="min-width:120px;">   
                    </th>
                </tr> 
            </thead>
            <tbody>            
    ...
    $(function () {
        table = $('.datatable').DataTable({
            ...
            "language": {
                ...
                "zeroRecords": "Pas de remédiations trouvées!",
        ...
        table.buttons().container().appendTo( '#remediations_table_wrapper .col-sm-6:eq(0)' );        
    });
