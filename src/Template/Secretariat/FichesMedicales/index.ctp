<div class="titre">
    <span>Fiches médicales - Liste globale des élèves</span>
</div>
        ...
        <table id="inscriptions_table" class="table datatable hover compact" >
            ...
            <?php $i=0; foreach($eleves as $eleve): ?>
                <tr>
                    ...
                    <td class="actions">
                        <?php if(sizeof($eleve->renseignement_valeurs)>0)
                            echo $this->Html->link('<i class="icone glyphicon glyphicon-eye-open"></i>', ['action' => 'fiche', $eleve->id], ['escape'=> false,'class'=>"btn btn-xs btn-default"]);
                            else echo $this->Html->link('<i class="icone glyphicon glyphicon-plus text-primary"></i>', ['action' => 'enregistrer', $eleve->id], ['escape'=> false,'class'=>"btn btn-xs btn-default"]);
                        ?>
                    </td>
                </tr>
            <?php $i++; endforeach;?>
