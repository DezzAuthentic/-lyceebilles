            ...
            <table class="datatable table-striped compact">
                ...
                <tbody id="lignes">
                    <?php $i=0; foreach($tests as $test):?>
                        <tr>
                            <td><input type="hidden" name="lignes[<?=$i?>][id]" value="<?=$test->id?>"><input type='hidden' name='lignes[<?=$i?>][matiere_id]' value='<?=$test->matiere_id?>'><?=$test->matiere->nom?></td>
                            <td><input type='date' name='lignes[<?=$i?>][date]' value='<?=$test->date->format('Y-m-d')?>' readonly class='form-control input-sm'></td>
                            <td><input type='text' name='lignes[<?=$i?>][heure]' value='<?=$test->heure?>' readonly class='form-control input-sm'></td>
                            <td><input type='text' name='lignes[<?=$i?>][duree]' value='<?=$test->duree?>' readonly class='form-control input-sm'></td>
...
<script>
    ...
    function ajouterLigne(id, libelle, quantite, prix){
        ...
        cell1 = "<input type='hidden' name='lignes["+i+"][id]' value=''> <input type='hidden' name='lignes["+i+"][matiere_id]' value='"+id+"'> "+libelle;
