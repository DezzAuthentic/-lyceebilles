...
class FacturesController extends AppController
{
    ...
    public function detailsMois($inscription_id=null,$mois_id=null, $frais_id=null){

        if($mois_id){
            $mois = $this->Mois->get($mois_id,[
                'contain' => [
                    'Factures.Reglements.Users.Employes',
                    'Factures.Inscriptions.Eleves',
                    'Factures.Inscriptions.Promotions',
                    'Factures.Frais' => [
                        'conditions' => ['Frais.id' => $frais_id]
                    ],
            ...
            $factures = $this->Factures->find("all", [
                'contain' => [
                    'Reglements.Users.Employes',
                    'Inscriptions.Eleves',
                    'Inscriptions.Promotions',
                    'Inscriptions',
                    'Reglements.Factures.Frais'
                ],
        ...
        $reglements = $this->Factures->Reglements->find('all',[
            'contain' => ['Users.Employes',"Factures", "Factures.Frais"],
            'conditions' => ['Factures.inscription_id' => $inscription_id, "Factures.mois_id IS" => $mois_id, "Factures.frais_id" => $frais_id]
        ]);
        ...
        $this->set(compact('mois','reglements'));

    }

    public function parEleveMois($inscription_id, $month=null, $moisPrecedents=null){
        $etab = $this->getEtablissement();
        $this->loadModel("Mois");
        $this->loadModel("Types");
    ...
    public function imprimer($inscription_id,$ordre1=null,$ordre2=null){
        ...
        $inscription = $this->Factures->Inscriptions->get($inscription_id,[
            'contain' => ['Eleves','Promotions','Affectations.Groupes', 'Promotions.Annees']
        ]);
